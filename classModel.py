...
def load_data(mode='train'):
    ...
    data = json.load(open("data/data65162/thousand.json"))
...
def train():
    with fluid.dygraph.guard():
        ...
        # 保存模型参数
        fluid.save_dygraph(model.state_dict(), 'MSOA')
...
def load_image(img_path):
    ...
    im = np.array(im).reshape(1, 1, 75, 75).astype(np.float32)

    # 图像归一化
    im = 1.0 - im / 255.
    return im
def check(imgpath:str,modelName="MSOA"):
    with fluid.dygraph.guard():
        model = MNIST()
        params_file_path = "./model"+"/"+modelName
        img_path = imgpath
        # 加载模型参数
        model_dict, _ = fluid.load_dygraph("./model/"+modelName)
        ...
        #print("{0}的预测结果为:{1} ".format(imgpath,lab[0][-1]))
        return lab[0][-1]
